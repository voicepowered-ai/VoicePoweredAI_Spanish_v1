...
parser.add_argument(
    "--fix_duration",
    type=float,
    help=f"Fix the total duration (ref and gen audios) in seconds, default",
)
...
model = args.model or config.get("model", "spanish")
...
if model == "F5-TTS":
    ...
    if not ckpt_file:  # path not specified, use local path
        ckpt_file = "ckpts/spanish/model_esp.safetensors"
